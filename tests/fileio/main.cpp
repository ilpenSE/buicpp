...
void print_file(const icl::io::File& file) {
  printf("Properties of %s:\n", file.name.c_str());
  printf("Size: %zu\n", file.size);
  printf("Type: %s\n", to_string(file.type));
  if (file.type == icl::io::FileType::SYMLINK) {
...
int main() {
  ...
  icl::io::read_entire_file(NOTES_FILE).match(
  [](auto file){
    print_file(file);
  },
  [](auto err){
  ...
  printf("------------------------------\n");
  printf("reading entire file: %s\n", TODOS_FILE);
  icl::io::read_entire_file(TODOS_FILE, false).match(
  [](auto file){
    print_file(file);
  },
  [](auto err){
